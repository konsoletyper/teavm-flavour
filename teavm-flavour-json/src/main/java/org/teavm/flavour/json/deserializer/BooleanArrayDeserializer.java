 ...
 */
public class BooleanArrayDeserializer extends NullableDeserializer {
    @Override
    public Object deserializeNonNull(JsonDeserializerContext context, Node node) {
        ...
        boolean[] array = new boolean[arrayNode.size()];
        for (int i = 0; i < array.length; ++i) {
            array[i] = JSON.deserializeBoolean(arrayNode.get(i));
        }
